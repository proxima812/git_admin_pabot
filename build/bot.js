...
async function fetchMarkdownFileContent(fileName) {
    const response = await axios_1.default.get(`https://api.github.com/repos/${process.env.GITHUB_REPO}/contents/src/content/posts/${fileName}`, {
        headers: {
            Authorization: `Bearer ${process.env.GITHUB_TOKEN}`,
            Accept: "application/vnd.github.v3+json",
        },
...
`;
    ...
    await axios_1.default.put(`https://api.github.com/repos/${process.env.GITHUB_REPO}/contents/src/content/posts/${fileName}`, {
...
bot.on("message:text", async (ctx) => {
    ...
    if (session.isNewPost) {
        ...
        await ctx.reply("Команда не распознана. Используйте /new_post для создания нового поста.");
    }
});
...
bot.on("callback_query:data", async (ctx) => {
    ...
    await ctx.reply(`Пост ${fileName}:\n\n` +
        `*title*: ${post.title}\n` +
        `*description*: ${post.description}\n` +
        `*datePublished*: ${post.datePublished}\n\n` +
        `${post.content}`, { parse_mode: "Markdown", reply_markup: keyboard });
});
// Функция для обновления атрибутов
async function updateAttribute(ctx, attribute) {
    const session = userSessions[ctx.chat.id];
    if (!session)
        return ctx.reply("Сначала выберите пост с помощью команды /posts.");
...
bot.on("message:text", async (ctx) => {
    const session = userSessions[ctx.chat.id];
    if (!session || !session.editingAttribute)
        return;
...
exports.default = grammy_1.webhookCallback(bot, "std/http");
